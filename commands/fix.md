---
description: Investigate a VibeCheck issue and produce a concrete fix plan. Fetches the issue context (title, description, evidence, suggested action) from the dashboard by ID, reads the relevant code, and presents step-by-step next steps. Use when the user provides a VibeCheck issue ID (e.g. B1, B2) to investigate and fix.
