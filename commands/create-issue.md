...
3. Report back to the user:
   ...
   - Show the issue's assigned ID in `PREFIX-N` format (e.g. `VC-405`) — this is what you'll pass to `/vibecheck:fix` or `/vibecheck:resolve`
...
- Use `/vibecheck:resolve VC-401` to remove the alert once the underlying problem is resolved
