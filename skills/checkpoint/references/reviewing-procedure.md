...
- **Instructions**: Review only the provided diff for correctness bugs, fragile assumptions, unhandled edge cases, error handling gaps, security risks, concurrency issues, performance risks, breaking API changes, and missing validation. POST findings to the VibeCheck server at `<VibeCheck_URL>/api/push/vc-review` (resolve the URL from the `VIBECHECK_API_URL` environment variable, or `~/.config/vibecheck/config` → `api_url=...` line, falling back to `http://localhost:8420`). Set `staged_files` to the scoped file list. Do NOT make code changes or dismiss issues — your only job is to analyze and report. Use the payload schema in `references/vc-review-schema.md`.
